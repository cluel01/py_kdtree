...
tree = KDTree(leaf_size=leaf_size,path="./run",dtype="float64",inmemory=False)

tree.fit(X)

point = np.array([20,5],dtype=np.float64)

#inds,pts,lv,t = tree.query_box(np.array([0,0,0]),np.array([10,10,10]),index_only=False)
inds,distances,dtime = tree.query_point_cy(point,k=5)
print(inds)
print(distances)
print(dtime)
